...
for(var i = 0; i < l; i++) {
  images[i].addEventListener("click", function(i) {
    ...
    var route = renameImgSrc(currentImg.src);
    ...
    const first = renameImgSrc(images[0].src), last = renameImgSrc(images[l-1].src);
    ...
    function prev() {
      ...
      imgItem.innerHTML = '<img src="' + renameImgSrc(prevImg.src) + '">';
      currentImg = currentImg.previousElementSibling;
      var mainImg = renameImgSrc(document.querySelector(".gg-image > img").src) ;
    ...
    function next() {
      ...
      imgItem.innerHTML = '<img src="' + renameImgSrc(nextImg.src) + '">';
...
function renameImgSrc(src) {
  return src.split('.jpg')[0] + '_large' + '.jpg';
}
